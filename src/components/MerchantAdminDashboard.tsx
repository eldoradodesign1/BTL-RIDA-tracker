...
import { Activity, BarChart3, Banknote, CheckCircle2, RefreshCw, Target, TrendingUp, UsersRound, PieChart as PieChartIcon } from 'lucide-react';
...
export const MerchantAdminDashboard: React.FC<MerchantAdminDashboardProps> = ({ onOpenManagement, podiumSlot, pendingFundRequestCount = 0, onOpenFundRequests }) => {
  ...
  return <div className="space-y-4 pb-4">
    ...
    <section className="grid grid-cols-1 gap-4 lg:grid-cols-2">
      <article className="glass-card relative overflow-hidden p-4 rounded-3xl border border-white/10 bg-zinc-950/60 backdrop-blur-xl shadow-xl">
        <div className="merchant-chart-glow pointer-events-none absolute -right-10 -top-12 h-32 w-32 rounded-full bg-violet-500/[0.12] blur-3xl"/>
        <div className="relative flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 rounded-xl bg-violet-500/15 border border-violet-500/30 text-violet-400 flex items-center justify-center">
              <PieChartIcon size={15}/>
            </div>
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.16em] text-violet-300">Anneau de couverture</p>
              <h2 className="mt-0.5 text-sm font-black text-white">État des POS</h2>
            </div>
          </div>
          <span className={`rounded-xl border border-white/10 bg-white/5 px-2.5 py-1 text-[10px] font-black ${percentageColor(summary.campaignExecutionRate)}`}>{summary.campaignExecutionRate}%</span>
        </div>
        <div className="relative mt-2 h-56">
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              <Pie data={summary.donut} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={58} outerRadius={84} paddingAngle={4} cornerRadius={9} isAnimationActive={false}>
                {summary.donut.map((entry) => <Cell key={entry.name} fill={entry.color} style={{ filter: `drop-shadow(0 0 8px ${entry.color}88)` }}/>)}
              </Pie>
              <Tooltip contentStyle={{ backgroundColor: '#090d16', borderColor: 'rgba(255,255,255,0.12)', borderRadius: '14px', fontSize: '11px', boxShadow: '0 12px 25px rgba(0,0,0,0.5)' }}/>
            </PieChart>
          </ResponsiveContainer>
          <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
            <b className="text-2xl font-black text-white">{summary.campaignExecutionRate}%</b>
        ...
        <div className="flex justify-center gap-2 text-[9px] font-black uppercase">
          {summary.donut.map((item) => (
            <span key={item.name} className="flex items-center gap-1.5 rounded-xl border border-white/5 bg-white/[0.04] px-2.5 py-1 text-gray-300">
              <i className="h-2 w-2 rounded-full shadow-[0_0_6px_currentColor]" style={{ backgroundColor: item.color, color: item.color }}/>
      ...
      <article className="glass-card relative overflow-hidden p-4 rounded-3xl border border-white/10 bg-zinc-950/60 backdrop-blur-xl shadow-xl">
        <div className="merchant-chart-glow pointer-events-none absolute -left-10 -bottom-12 h-32 w-32 rounded-full bg-cyan-500/[0.12] blur-3xl"/>
        <div className="relative flex items-center gap-2">
          ...
          <div>
            <p className="text-[10px] font-black uppercase tracking-[0.16em] text-cyan-300">Histogramme de cadence</p>
            <h2 className="mt-0.5 text-sm font-black text-white">POS réalisés par BA</h2>
          </div>
        </div>
        <div className="relative mt-3 h-64">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={summary.byBa} margin={{ top: 10, right: 4, left: -22, bottom: 8 }}>
              <defs>
                <linearGradient id="merchBarPos" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor="#06B6D4" />
                  <stop offset="100%" stopColor="#0284C7" />
                </linearGradient>
                <linearGradient id="merchBarTarget" x1="0" y1="0" x2="0" y2="1">
              ...
              <Tooltip contentStyle={{ backgroundColor: '#090d16', borderColor: 'rgba(255,255,255,0.12)', borderRadius: '14px', fontSize: '11px', boxShadow: '0 12px 25px rgba(0,0,0,0.5)' }}/>
              <Bar dataKey="pos" name="POS réalisés" fill="url(#merchBarPos)" radius={[7,7,0,0]} isAnimationActive={false}/>
              <Bar dataKey="target" name="Objectif" fill="url(#merchBarTarget)" radius={[7,7,0,0]} isAnimationActive={false}/>
            </BarChart>
          </ResponsiveContainer>
        </div>
      </article>
    ...
    <section className="glass-card relative overflow-hidden p-4 rounded-3xl border border-white/10 bg-zinc-950/60 backdrop-blur-xl shadow-xl">
      <div className="merchant-chart-glow pointer-events-none absolute -right-14 -bottom-16 h-44 w-44 rounded-full bg-emerald-500/[0.10] blur-3xl"/>
      <div className="relative flex items-center gap-2">
        ...
        <div>
          <p className="text-[10px] font-black uppercase tracking-[0.16em] text-emerald-300">Courbe de progression</p>
          <h2 className="mt-0.5 text-sm font-black text-white">POS visités sur les 7 derniers jours</h2>
        </div>
      </div>
      <div className="relative mt-3 h-64">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={summary.timeline} margin={{ top: 12, right: 8, left: -20, bottom: 3 }}>
            <defs>
              <linearGradient id="merchLineGrad" x1="0" y1="0" x2="1" y2="0">
                <stop offset="0%" stopColor="#06B6D4" />
            ...
            <Line type="monotone" dataKey="visits" name="POS visités" stroke="url(#merchLineGrad)" strokeWidth={3} dot={{ r: 4, fill: '#10B981' }} activeDot={{ r: 6 }} isAnimationActive={false}/>
            <Line type="monotone" dataKey="target" name="Objectif actif" stroke="#F59E0B" strokeWidth={2} strokeDasharray="6 5" dot={false} isAnimationActive={false}/>
          </LineChart>
        </ResponsiveContainer>
      </div>
      <div className="relative mt-2 flex items-center justify-between rounded-2xl border border-white/10 bg-white/[0.035] px-3 py-2 text-[10px]">
        <span className="font-bold text-gray-400">Transactions aujourd’hui</span>
        <b className="text-emerald-100">{summary.transactionsToday.toLocaleString('fr-FR')}</b>
        <span className="font-bold text-gray-400">Objectifs pilotables dans Gestion</span>
