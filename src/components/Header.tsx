...
import { Bell, LogOut, Shield, RefreshCw, Palette, Camera, X, BriefcaseBusiness, Banknote, FileSpreadsheet } from 'lucide-react';
...
import { isAppScriptConfigured } from '../utils/appScriptApi';
...
export const Header: React.FC<HeaderProps> = ({
  ...
  const visibleCampaigns = campaignOptions || [
    { key: 'rida-installation' as const, label: 'RIDA-Installation', note: 'Installations Lubumbashi' },
  ];
      ...
      <div className="flex items-start justify-between gap-2 sm:gap-3">
        ...
        >
          {photoSrc ? (
            ...
            <span className="px-1 text-[8px] font-black uppercase tracking-wide leading-[1.05] text-center text-emerald-100">Check-in Hub</span>
          )}
        ...
        />
          ...
          <div className="flex items-center space-x-2">
            <span className="font-black text-sm sm:text-lg tracking-tight brand-text truncate text-white">
            ...
            <span className={`text-xs sm:text-sm font-black mt-0.5 px-2.5 py-1 rounded-full inline-flex items-center gap-2 w-fit max-w-full truncate ${
              isDarkTheme ? 'bg-white/10 text-emerald-300 border border-white/20' : 'bg-emerald-50 text-emerald-800 border border-emerald-200'
            }`} title={user.name}>
            ...
            <span className="hidden sm:inline text-[9px] text-zinc-400 font-bold tracking-wide uppercase opacity-80">
              Campagne <b className="text-[#00D084] font-black">RIDA Lubumbashi</b>
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center justify-end gap-1.5 sm:gap-2 shrink-0" data-header-actions>
                  ...
                  {visibleCampaigns.map((campaign) => (
                    <button
                      ...
                      className={`w-full text-left px-3 py-2.5 rounded-xl transition-all ${
                        activeCampaign === campaign.key ? 'bg-emerald-500 text-zinc-950 font-black' : (isDarkTheme ? 'hover:bg-white/10 text-gray-100' : 'hover:bg-zinc-100 text-zinc-700')
                      }`}
                    >
          ...
          {onSetTheme && (
            <div className="relative">
              ...
              {showThemeMenu && (
                <div className={`app-popover absolute right-0 top-11 w-44 border rounded-2xl p-1.5 shadow-2xl z-50 backdrop-blur-2xl ${
                  isDarkTheme ? 'bg-zinc-900/95 border-white/10 text-white' : 'bg-white/95 border-zinc-200 text-zinc-900'
                }`}>
                  {[
                    { key: 'anthracite', label: 'Anthracite', dot: '#10B981' },
                    { key: 'rubis', label: 'Rubis', dot: '#F43F5E' },
                    { key: 'silver', label: 'Silver', dot: '#38BDF8' },
                    { key: 'diamond', label: 'Diamond', dot: '#94A3B8' },
                    { key: 'sapphire', label: 'Sapphire', dot: '#60A5FA' },
                    { key: 'ambre', label: 'Ambre', dot: '#F59E0B' }
                  ].map(opt => (
                    <button
                      key={opt.key}
                      onClick={() => {
                      ...
                      className={`w-full flex items-center justify-between px-3 py-2 rounded-xl text-xs font-black uppercase transition-all ${
                        theme === opt.key
                    ...
                    >
                      <span>{opt.label}</span>
                      <span className="w-2.5 h-2.5 rounded-full shadow-[0_0_6px_currentColor]" style={{ backgroundColor: opt.dot, color: opt.dot }} />
                    </button>
                  ))}
                </div>
              )}
          ...
          {onOpenSystemConfig && (
            <button
              ...
              title={`Base de données Google Sheets (${isAppScriptConfigured() ? 'Connecté' : 'Mode Local'})`}
            >
              <FileSpreadsheet className="w-4 h-4" />
              <span
                className={`absolute -top-0.5 -right-0.5 w-2 h-2 rounded-full border border-black ${
                  isAppScriptConfigured() ? 'bg-emerald-400' : 'bg-amber-400'
                }`}
              />
            </button>
          )}
          ...
          <div className="relative">
            ...
            >
              ...
              {unreadCount > 0 && (
                <span className="absolute -top-1 -right-1 w-5 h-5 bg-rose-500 text-white rounded-full text-[10px] font-black flex items-center justify-center border-2 border-zinc-950 shadow-[0_0_8px_rgba(244,63,94,0.6)] animate-pulse">
                  {unreadCount}
                </span>
              )}
            </button>
            ...
            {/* Notifications Panel */}
            {showNotifPanel && (
              <div className={`app-popover absolute right-0 top-12 w-80 border rounded-3xl p-4 shadow-2xl z-50 backdrop-blur-2xl animate-pop ${
                isDarkTheme ? 'bg-zinc-900/95 border-white/10 text-white' : 'bg-white/95 border-zinc-200 text-zinc-900'
              }`}>
