  ...
  return <div className="rida-shell">
    ...
    <div className="ops-main">
      ...
      <main>
        ...
        {isAgent&&page==='account'&&<div className="ops-page"><SectionTitle eyebrow="MON COMPTE" title={user.name} description={`${roleLabel[user.role]} · ${user.phone}`}/><section className="ops-panel profile-panel">{getTodayCheckinPhoto(user.id)?<img className="profile-avatar profile-photo" src={getTodayCheckinPhoto(user.id)!} alt="Photo du pointage" />:<div className="profile-avatar">{initials(user.name)}</div>}<div><h2>{user.name}</h2><p>{user.phone}</p><StatusPill status="Compte actif"/></div></section></div>}
    ...
    <div className="mobile-nav">{(isAdmin ? nav.filter(x=>['overview','people','field','reports','settings'].includes(x[0])) : nav.slice(0,5)).map(([key,label,Icon])=><button key={key} className={page===key?'active':''} onClick={()=>setPage(key)}><Icon size={18}/><span>{label}</span></button>)}</div>
