...
export const RidaOpsApp:React.FC<Props>=({user,users,shops,online,syncPendingCount,onRefresh,onLogout,onOpenSystemConfig,onSimulateRole,simulationActive,onExitSimulation})=>{
  ...
  return <div className="rida-shell">
    ...
    <div className="mobile-nav">{(isAdmin ? [nav[0],nav[1],nav[4],nav[5],nav[7]] : nav.slice(0,5)).filter(Boolean).map(([key,label,Icon])=><button key={key} className={page===key?'active':''} onClick={()=>setPage(key)}><Icon size={18}/><span>{label}</span></button>)}</div>
