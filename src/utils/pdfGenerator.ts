...
export function buildAgentReportHtml(d: PDFReportData): string {
  ...
  const gpsInHtml = gpsInUrl
    ? `<a href="${gpsInUrl}" target="_blank" rel="noopener noreferrer" style="display:inline-block; margin-top:6px; font-size:9px; color:#00873D; font-weight:bold; text-decoration:none; border:1px solid #00C853; padding:4px 10px; border-radius:6px; background:#fff;">📍 Vérifier GPS (Carte)</a>`
  ...
  const gpsOutHtml = gpsOutUrl
    ? `<a href="${gpsOutUrl}" target="_blank" rel="noopener noreferrer" style="display:inline-block; margin-top:6px; font-size:9px; color:#00873D; font-weight:bold; text-decoration:none; border:1px solid #00C853; padding:4px 10px; border-radius:6px; background:#fff;">📍 Vérifier GPS (Carte)</a>`
    : `<span style="display:inline-block; margin-top:6px; font-size:9px; color:#999; font-weight:bold; border:1px solid #ddd; padding:4px 10px; border-radius:6px; background:#fafafa;">Coordonnées non disponibles</span>`;
  ...
  return `
    ...
    <div style="background:linear-gradient(135deg,#064e3b 0%,#062a22 48%,#00d084 100%); color:#fff; border-radius:20px; padding:18px; margin-bottom:16px; box-shadow:0 12px 30px rgba(2,6,23,0.35); position:relative; overflow:hidden; font-family:'Inter','Segoe UI',Roboto,Helvetica,Arial,sans-serif;">
      ...
      <table style="width:100%; border-collapse:collapse;">
        <tr>
          <td style="width:90px; vertical-align:top;">
            <div style="width:80px; height:80px; border-radius:16px; border:3px solid #00D084; overflow:hidden; display:flex; align-items:center; justify-content:center; background:#0b1220;">
              ${profilePhoto
                ...
                : `<span style="font-size:10px; font-weight:900; color:#cbd5e1; text-transform:uppercase; text-align:center;">Photo</span>`}
            </div>
          </td>
          <td style="vertical-align:top; padding-left:14px;">
            <div style="font-size:10px; font-weight:800; letter-spacing:1px; text-transform:uppercase; color:#a7f3d0;">🚕 RIDA FIELD TRACKER • CAMPAGNE RIDA-INSTALLATION</div>
            <div style="font-size:24px; line-height:1.05; font-weight:900; text-transform:uppercase; margin-top:4px;">${escapeHtml(d.agentName)}</div>
            <div style="font-size:12px; color:#e2e8f0; margin-top:6px;">Hub: <b>${escapeHtml(d.shopName)}</b> • Date: <b>${escapeHtml(d.date)}</b></div>
          </td>
        </tr>
      </table>
    </div>
      ...
      <tbody>
        ${safeLeads.length > 0 ? safeLeads.map((l, i) => `
          <tr style="border-bottom:1px solid #eee; font-size:10px; background:${i % 2 === 0 ? '#fff' : '#fcfcfc'};">
            ...
            <td style="padding:6px; font-family:monospace; font-weight:bold; color:#00873D;">${escapeHtml(l.msisdn)}</td>
    ...
    <div style="margin-top:30px; text-align:center; font-size:9px; color:#888; text-transform:uppercase; letter-spacing:1px; font-weight:bold;">
      RIDA TRACKER — CAMPAGNE INSTALLATION LUBUMBASHI
    </div>
  `;
}
...
function buildSupervisorReportPages(d: PDFSupervisorData): string[] {
  ...
  const cover = `
    <div style="background:linear-gradient(140deg,#064e3b 0%,#062a22 48%,#00d084 100%); border-radius:24px; padding:24px; color:#fff; margin-bottom:18px;">
      <div style="font-size:10px; text-transform:uppercase; letter-spacing:1px; color:#a7f3d0; font-weight:800;">Rapport de supervision • RIDA Lubumbashi</div>
      ...
      <div style="font-size:11px; color:#dbeafe; margin-top:10px;">A4 • Synthèse équipe • ${d.team.length} agent(s) • ${reportCount} rapport(s)</div>
    </div>
  `;
  ...
  const summary = `
    <table style="width:100%; border-spacing:10px; margin-left:-10px; margin-bottom:10px;">
      <tr>
        ...
        <td style="width:34%;"><div style="background:#f0fdf4; border:1px solid #a7f3d0; border-radius:14px; padding:12px; text-align:center;"><div style="font-size:9px; color:#065f46; text-transform:uppercase; font-weight:800;">Installations équipe</div><div style="font-size:24px; font-weight:900; color:#059669;">${totalLeads}</div></div></td>
      </tr>
    </table>
  `;
...
function buildAdminBatchReportPages(d: PDFAdminBatchData): string[] {
  ...
  const cover = `
    <div style="background:linear-gradient(135deg,#064e3b 0%,#062a22 46%,#00d084 100%); color:#fff; border-radius:24px; padding:24px; margin-bottom:18px;">
      <div style="font-size:10px; text-transform:uppercase; letter-spacing:1px; color:#a7f3d0; font-weight:800;">RIDA FIELD TRACKER • COMPILATION PÉRIODE LUBUMBASHI</div>
      <div style="font-size:34px; font-weight:900; margin-top:6px;">Rapport Compilé RIDA</div>
  ...
  const summaryCards = `
    <table style="width:100%; border-spacing:10px; margin-left:-10px; margin-bottom:10px;">
      <tr>
        <td style="width:25%;"><div style="background:#f0fdf4; border:1px solid #bbf7d0; border-radius:14px; text-align:center; padding:14px;"><div style="font-size:9px; color:#065f46; text-transform:uppercase; font-weight:800;">Android</div><div style="font-size:24px; font-weight:900; color:#059669; margin-top:3px;">${totals.privilege}</div></div></td>
        <td style="width:25%;"><div style="background:#eff6ff; border:1px solid #bfdbfe; border-radius:14px; text-align:center; padding:14px;"><div style="font-size:9px; color:#1e3a8a; text-transform:uppercase; font-weight:800;">iOS (Apple)</div><div style="font-size:24px; font-weight:900; color:#1d4ed8; margin-top:3px;">${totals.roaming}</div></div></td>
        <td style="width:25%;"><div style="background:#fffbeb; border:1px solid #fde68a; border-radius:14px; text-align:center; padding:14px;"><div style="font-size:9px; color:#92400e; text-transform:uppercase; font-weight:800;">Autres</div><div style="font-size:24px; font-weight:900; color:#78350f; margin-top:3px;">${totals.bundles}</div></div></td>
        <td style="width:25%;"><div style="background:#064e3b; border:1px solid #00d084; border-radius:14px; text-align:center; padding:14px;"><div style="font-size:9px; color:#a7f3d0; text-transform:uppercase; font-weight:800;">Total Rapports</div><div style="font-size:24px; font-weight:900; color:#ffffff; margin-top:3px;">${rows.length}</div></div></td>
      </tr>
    </table>
  `;
