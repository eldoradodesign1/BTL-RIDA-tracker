...
function mergeUsersWithSeedData(storedUsers: User[]): User[] {
  ...
  merged.forEach((user, index) => {
    if (user && user.phone !== undefined && user.phone !== null && typeof user.phone !== 'string') {
  ...
  (Array.isArray(storedUsers) ? storedUsers : []).forEach((candidate) => {
    if (!candidate) return;
    if (candidate.phone !== undefined && candidate.phone !== null && typeof candidate.phone !== 'string') {
      candidate.phone = String(candidate.phone);
    }
    ...
    merged[existingIndex] = {
      ...
      phone: String(candidate.phone || existing.phone || ''),
...
function normalizeLoginToken(value: unknown): string {
...
export function normalizePhoneMSISDN(phone: unknown): string {
...
export function authenticate(phone: unknown, password_hash: string): { success: boolean; user?: User; message?: string } {
  const users = getUsers();
...
export function getDashboardData(filters: { start?: string; end?: string; agentId?: string }) {
  ...
  const pieData = [
    { name: 'Privilège', value: priv, color: '#10B981' },
    { name: 'Roaming', value: roam, color: '#F59E0B' },
    { name: 'Bundles', value: bund, color: '#06B6D4' }
  ];
