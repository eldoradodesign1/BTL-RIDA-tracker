...
export function isValidMsisdn(phone: unknown): boolean {
...
export function formatMsisdn(phone: unknown): string {
...
export const isValidDRCPhone = isValidMsisdn;

export function normalizeDRCPhone(phone: unknown): string {
  let cleaned = cleanPhoneNumber(phone);
  if (cleaned.startsWith('243') && cleaned.length === 12) {
    cleaned = '0' + cleaned.substring(3);
  }
