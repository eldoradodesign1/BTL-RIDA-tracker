...
import {
  ...
  fetchShopsFromGoogleSheets,
  fetchCheckinsFromGoogleSheets,
  fetchLeadsFromGoogleSheets,
  fetchReportsFromGoogleSheets
} from './appScriptApi';
...
export function isSupabaseConfigured(overrides?: Partial<SupabaseConfig>): boolean {
  return isAppScriptConfigured() || !!getSupabaseConfig(overrides);
}
...
export async function syncLocalDataToSupabase(payload: {
  ...
  if (isAppScriptConfigured()) {
    const res = await syncLocalDataToGoogleSheets(payload);
    return {
      users: res.users || 0,
      shops: res.shops || 0,
      checkins: res.checkins || 0,
      leads: res.leads || 0,
      reports: res.reports || 0,
      notifications: res.notifications || 0,
      chatMessages: res.chatMessages || 0
    };
  }

  const client = getSupabaseClient(overrides);
  if (!client) {
    throw new Error('Supabase ou Google Apps Script n\'est pas configuré.');
  }
...
export async function uploadPhotoToSupabase(fileOrDataUrl: string, bucket: string, pathPrefix = 'checkins', overrides?: Partial<SupabaseConfig>): Promise<string> {
  if (isAppScriptConfigured()) {
    return await uploadPhotoToGoogleDrive(fileOrDataUrl, `${pathPrefix}_${Date.now()}.jpg`);
  }

  const client = getSupabaseClient(overrides);
  if (!client) {
    // Si ni Apps Script ni Supabase ne sont connectés, renvoyer la data URL locale
...
export async function fetchUsersFromSupabase(): Promise<User[]> {
  if (isAppScriptConfigured()) {
    return await fetchUsersFromGoogleSheets();
  }
  ...
  return (data || []).map((u: any) => ({
    ...
    permanentShopId: u.permanent_shop_id || u.permanentShopId,
    userCategory: u.user_category || u.userCategory || undefined,
    authUserId: u.auth_user_id || u.authUserId || undefined,
...
export async function fetchShopsFromSupabase(): Promise<Shop[]> {
  if (isAppScriptConfigured()) {
    return await fetchShopsFromGoogleSheets();
  }
...
export async function fetchLeadsFromSupabase(): Promise<Lead[]> {
  if (isAppScriptConfigured()) {
    return await fetchLeadsFromGoogleSheets();
  }
...
export async function fetchCheckinsFromSupabase(): Promise<Checkin[]> {
  if (isAppScriptConfigured()) {
    return await fetchCheckinsFromGoogleSheets();
  }
...
export async function fetchReportsFromSupabase(): Promise<DailyReport[]> {
  if (isAppScriptConfigured()) {
    return await fetchReportsFromGoogleSheets();
  }
