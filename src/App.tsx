...
export default function App() {
  ...
  useEffect(() => {
    ...
    const emitStrongFundAlert = (requests: Array<{ id: string; baName: string; amount: number; posLabel: string }>, notifySystem = false) => {
      ...
      if (notifySystem && first) void showFundRequestSystemNotification(`Demande de fonds - ${first.baName}`, `${first.amount.toLocaleString('fr-FR')} FC pour ${first.posLabel}`);
...
const todayLeads =
  ...
  const handleSimulateRoleChange = (role: UserRole) => {
    ...
    const target = role === 'agent'
      ? (users.find((u) => u.name.trim().toLowerCase() === 'agent test') || users.find((u) => u.role === 'agent'))
      : role === 'supervisor'
        ? (users.find((u) => u.name.trim().toLowerCase().includes('ntalu')) || users.find((u) => u.role === 'supervisor'))
        : role === 'admin'
          ? (users.find((u) => u.name.trim().toLowerCase().includes('izamaboko')) || users.find((u) => u.role === 'admin'))
          : (users.find((u) => u.role === 'super_admin') || users.find((u) => u.role === 'admin'));
    if (target) {
  ...
  const themeSurfaceStyle = theme === 'anthracite'
    ? { backgroundColor: '#07090e', backgroundImage: 'radial-gradient(circle at 20% 10%, rgba(16, 185, 129, 0.12) 0%, transparent 45%), radial-gradient(circle at 80% 80%, rgba(6, 182, 212, 0.10) 0%, transparent 50%), linear-gradient(180deg, #07090e 0%, #0d121c 100%)', color: '#f8fafc' }
    : theme === 'rubis'
      ? { backgroundColor: '#0b070c', backgroundImage: 'radial-gradient(circle at 20% 10%, rgba(225, 29, 72, 0.14) 0%, transparent 45%), radial-gradient(circle at 80% 80%, rgba(168, 85, 247, 0.10) 0%, transparent 50%), linear-gradient(180deg, #0b070c 0%, #120914 100%)', color: '#fff7f7' }
      : theme === 'silver'
        ? { backgroundColor: '#090c12', backgroundImage: 'radial-gradient(circle at 20% 10%, rgba(148, 163, 184, 0.12) 0%, transparent 45%), radial-gradient(circle at 80% 80%, rgba(56, 189, 248, 0.10) 0%, transparent 50%), linear-gradient(180deg, #090c12 0%, #0f1522 100%)', color: '#f8fafc' }
        : theme === 'diamond'
          ? { backgroundColor: '#f8fafc', backgroundImage: 'radial-gradient(circle at 20% 10%, rgba(226, 232, 240, 0.8) 0%, transparent 50%), radial-gradient(circle at 80% 80%, rgba(203, 213, 225, 0.6) 0%, transparent 50%), linear-gradient(180deg, #f8fafc 0%, #f1f5f9 100%)', color: '#0f172a' }
          : theme === 'sapphire'
            ? { backgroundColor: '#060b14', backgroundImage: 'radial-gradient(circle at 20% 10%, rgba(59, 130, 246, 0.14) 0%, transparent 45%), radial-gradient(circle at 80% 80%, rgba(6, 182, 212, 0.12) 0%, transparent 50%), linear-gradient(180deg, #060b14 0%, #0b1322 100%)', color: '#f8fbff' }
            : theme === 'ambre'
              ? { backgroundColor: '#0a0805', backgroundImage: 'radial-gradient(circle at 20% 10%, rgba(245, 158, 11, 0.14) 0%, transparent 45%), radial-gradient(circle at 80% 80%, rgba(217, 119, 6, 0.10) 0%, transparent 50%), linear-gradient(180deg, #0a0805 0%, #130f0a 100%)', color: '#fffef7' }
              : { backgroundColor: '#07090e', backgroundImage: 'radial-gradient(circle at 20% 10%, rgba(16, 185, 129, 0.12) 0%, transparent 45%), radial-gradient(circle at 80% 80%, rgba(6, 182, 212, 0.10) 0%, transparent 50%), linear-gradient(180deg, #07090e 0%, #0d121c 100%)', color: '#f8fafc' };
  ...
  return (
    ...
    >
      ...
      <Header
        ...
        onOpenSystemConfig={() => setIsSystemConfigurationOpen(true)}
